    ...
    code: `
      ...
      const MiniChallenge3 = ({ recipe }) => {
          ...
          if (!recipe) {
              ...
              return (
                  <div className={p- 6 bg- white rounded-lg shadow - md dollor{favorite ? 'border-4 border-yellow-400' : ''} }>
                      <h2>{recipe.name}</h2>
                      <p>{recipe.description}</p>
                      <div className="flex items-center gap-2 mb-4">
                          <button
                              onClick={toggleFavorite}
                              className={px - 3 py - 1 rounded - md dollor{favorite ? 'bg-yellow-500 text-white' : 'bg-gray-200 text-black'
      } }
                          >
