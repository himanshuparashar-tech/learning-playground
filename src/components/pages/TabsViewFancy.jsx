import React, { useState, useEffect, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { getTabs } from './tabsData.jsx';

export default function TabsViewFancy() {
    const [activeTab, setActiveTab] = useState(1);
    const [isLoading, setIsLoading] = useState(false);
    const [search, setSearch] = useState("");
    const [isLastTabActive, setIsLastTabActive] = useState(false);


    // ✅ For code/preview toggle
    const [viewMode, setViewMode] = useState({ 1: 'preview' });

    // ✅ For auto-scroll
    const tabRefs = useRef({});

    const setTabView = (tabId, mode) =>
        setViewMode(prev => ({ ...prev, [tabId]: mode }));

    const handleTabClick = (tabId) => {
        setActiveTab(tabId);
        setViewMode(prev => ({ ...prev, [tabId]: 'preview' }));
    };

    const tabs = getTabs();

    // ✅ SEARCH FILTER
    const filteredTabs = tabs.filter(tab =>
        tab.name.toLowerCase().includes(search.toLowerCase())
    );

    // ✅ LOADER EFFECT
    useEffect(() => {
        if (activeTab) {
            setIsLoading(true);
            const timer = setTimeout(() => setIsLoading(false), 400);
            return () => clearTimeout(timer);
        }
    }, [activeTab]);

    // ✅ AUTO SCROLL TO ACTIVE TAB
    useEffect(() => {
        const activeEl = tabRefs.current[activeTab];

        if (activeEl) {
            activeEl.scrollIntoView({
                behavior: "smooth",
                block: "nearest"
            });
        }
    }, [activeTab]);

    const handleLastTabToggle = () => {
        if (filteredTabs.length === 0) return;

        if (!isLastTabActive) {
            // ✅ Go to LAST tab
            const lastTabId = filteredTabs[filteredTabs.length - 1].id;
            handleTabClick(lastTabId);
        } else {
            // ✅ Go back to FIRST tab
            const firstTabId = filteredTabs[0].id;
            handleTabClick(firstTabId);
        }

        setIsLastTabActive(prev => !prev);
    };

    return (
        <div className='p-3 w-full mx-auto'>
            <div className='flex flex-col lg:flex-row gap-6 rounded-xl'>

                {/* ✅ LEFT SIDEBAR */}
                <div className='max-w-full lg:w-64 flex lg:flex-col rounded-xl bg-black/5 dark:bg-white/5 backdrop-filter backdrop-blur-lg pr-2'>

                    {/* ✅ SEARCH BAR */}
                    <div className="searchBar">
                        <input
                            type="text"
                            placeholder="Search tabs..."
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            className="my-3 px-3 py-2 rounded-lg border outline-none text-sm bg-white border-purple-500 border-2 w-full"
                        />
                    </div>

                    <div className="flex gap-2 px-2 mb-3">
                        {/* ✅ CLEAR SEARCH BUTTON */}
                        <button
                            onClick={() => setSearch("")}
                            className="flex-1 py-2 text-sm rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:opacity-90 transition"
                        >
                            Clear
                        </button>

                        {/* ✅ LAST TAB TOGGLE BUTTON */}
                        <button
                            onClick={handleLastTabToggle}
                            className="flex-1 py-2 text-sm rounded-lg bg-gray-200 hover:bg-gray-300 transition"
                        >
                            {isLastTabActive ? "First Tab" : "Last Tab"}
                        </button>
                    </div>


                    {/* ✅ TABS LIST */}
                    <div className="overflow-y-auto" style={{ maxHeight: 'calc(100vh - 266px)' }}>
                        {filteredTabs.map(tab => (
                            <button
                                key={tab.id}
                                ref={(el) => (tabRefs.current[tab.id] = el)}
                                onClick={() => handleTabClick(tab.id)}
                                className={`
                  relative group flex flex-wrap break-words items-center sm:w-full
                  lg:px-4 lg:py-3 px-2 py-2 min-w-[200px] transition-all
                  ${activeTab === tab.id
                                        ? 'text-white'
                                        : 'text-gray-500 hover:text-gray-800'}
                `}
                            >
                                {/* ✅ ACTIVE BACKGROUND */}
                                {activeTab === tab.id && (
                                    <motion.div
                                        layoutId='tabBackground'
                                        className='absolute inset-0 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg'
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                                        transition={{ duration: 0.2 }}
                                    />
                                )}

                                <div className='flex text-left items-center z-10'>
                                    <p className='font-medium text-sm'>{tab.name}</p>
                                </div>

                                {/* ✅ ACTIVE DOT */}
                                {activeTab === tab.id ? (
                                    <motion.div
                                        layoutId='activeDot'
                                        className='absolute right-3 w-2 h-2 rounded-full bg-white'
                                        initial={{ scale: 0 }}
                                        animate={{ scale: 1 }}
                                        transition={{ delay: 0.1 }}
                                    />
                                ) : (
                                    <div className='absolute right-3 w-2 h-2 rounded-full bg-gray-400/0 group-hover:bg-gray-400/30 transition-colors' />
                                )}
                            </button>
                        ))}

                        {/* ✅ EMPTY SEARCH RESULT */}
                        {filteredTabs.length === 0 && (
                            <p className="text-center text-sm text-gray-400 py-4">
                                No Tabs Found
                            </p>
                        )}
                    </div>
                </div>

                {/* ✅ RIGHT CONTENT PANEL */}
                <div className='flex-1 relative rounded-xl bg-white backdrop-filter backdrop-blur-lg shadow-lg overflow-auto fancy-view'
                    style={{ maxHeight: 'calc(100vh - 160px)', minHeight: 'calc(100vh - 278px)' }}>

                    {/* ✅ LOADER */}
                    <AnimatePresence>
                        {isLoading && (
                            <motion.div
                                key='loader'
                                className='absolute inset-0 z-20 flex items-center justify-center bg-white/90 backdrop-blur-sm'
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 0.7 }}
                                exit={{ opacity: 0 }}
                                transition={{ duration: 0.2 }}
                            >
                                <svg className='animate-spin h-8 w-8 text-indigo-600' xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24'>
                                    <circle className='opacity-25' cx='12' cy='12' r='10' stroke='currentColor' strokeWidth='4' />
                                    <path className='opacity-75' fill='currentColor'
                                        d='M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z' />
                                </svg>
                            </motion.div>
                        )}
                    </AnimatePresence>

                    {/* ✅ TAB CONTENT */}
                    <AnimatePresence mode='wait'>
                        <motion.div
                            key={activeTab}
                            initial={{ opacity: 0, x: 10 }}
                            animate={{ opacity: 1, x: 0 }}
                            exit={{ opacity: 0, x: -10 }}
                            transition={{ duration: 0.3 }}
                            className='p-6 bg-white'
                        >
                            <h3 className='text-lg font-semibold mb-4 text-gray-900'>
                                {tabs.find(t => t.id === activeTab)?.name}
                            </h3>

                            {/* ✅ CODE / PREVIEW TOGGLE */}
                            {tabs.find(tab => tab.id === activeTab)?.type === 'codePreview' ? (
                                <>
                                    <div className='mb-4 flex gap-2'>
                                        <button
                                            onClick={() => setTabView(activeTab, 'code')}
                                            className={`px-3 py-1 rounded ${viewMode[activeTab] === 'code' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}
                                        >
                                            💻 Code
                                        </button>

                                        <button
                                            onClick={() => setTabView(activeTab, 'preview')}
                                            className={`px-3 py-1 rounded ${viewMode[activeTab] !== 'code' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}
                                        >
                                            👀 Preview
                                        </button>
                                    </div>

                                    {viewMode[activeTab] === 'code' ? (
                                        <pre className='rounded bg-gray-100 p-4 overflow-auto text-sm'>
                                            <code>{tabs.find(tab => tab.id === activeTab).code}</code>
                                        </pre>
                                    ) : (
                                        <div>{tabs.find(tab => tab.id === activeTab).preview}</div>
                                    )}
                                </>
                            ) : (
                                <div>
                                    {tabs.find(tab => tab.id === activeTab)?.content || tabs[0].content}
                                </div>
                            )}
                        </motion.div>
                    </AnimatePresence>

                </div>
            </div>
        </div>
    );
}
